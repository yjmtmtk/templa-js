...
- **Build** (`npx @yjmtmtk/templa build`) — partials are inlined ahead of time into static HTML
...
<script src="https://cdn.jsdelivr.net/npm/@yjmtmtk/templa/templa.js"></script>
```

> jsDelivr serves `templa.min.js` by minifying the source on the fly, so there is no separate minified file to maintain. Pin a version with `@yjmtmtk/templa@0.0.1` if you want immutable URLs.
...
This pattern works identically with the build CLI — `npx @yjmtmtk/templa build` inlines the layout into the output and you can drop the script tags.
...
| | Runtime (`templa.js`) | Build (`npx @yjmtmtk/templa`) |
|---|---|---|
